...
public class restaurant {
	...
	static RestaurantManager mint = new RestaurantManager();
	static Scanner console = new Scanner(System.in);
	
	static Food[] menu = new Food[buildMenu(mint.readFile()).size()];

	public static ArrayList<Food> buildMenu(String menu){
		Scanner sc = new Scanner(menu);
		String collect = "";
		int price = 0;
		ArrayList<Food> me = new ArrayList<Food>();
		
		while(sc.hasNextLine()){
			String line = sc.nextLine();
			collect = line.split(";")[0];
			price = Integer.parseInt(line.split(";")[1].trim());
			me.add(new Food(collect,price));
		}
		return me;
	}
	...
	public static void printmenu() {
		...
		System.out.printf("|  %d.) %s\t %5d Bath.                  |\n", h + 1, menu[h].getName(), menu[h].getPrice());
		}
...
//calculate total price
	public static int total(){
		...
		for(int i =0;i<menu.length;i++){
			sum = sum+menu[i].getQuantity()*menu[i].getPrice();
		}
		return sum;
	}
	...
	public static void main(String[] args) {
		menu = buildMenu(mint.readFile()).toArray(menu);
		menu();
		while (true) {
			System.out.printf("\nEnter your Choice: ");
